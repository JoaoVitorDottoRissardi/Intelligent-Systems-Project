...
class Wangmendel():
    ...
    def generate_mfs(self):
        ...
        self.qpa_range = np.linspace(min(self.train["qpa"])-qpa_dif, max(self.train["qpa"])+qpa_dif, self.no_mf)
        self.pulse_range = np.linspace(min(self.train["pulse"])-pulse_dif, max(self.train["pulse"])+pulse_dif, self.no_mf)
        self.resp_range = np.linspace(min(self.train["resp"])-resp_dif, max(self.train["resp"])+resp_dif, self.no_mf)
        self.gravity_range = np.linspace(min(self.train["gravity"])-gravity_dif, max(self.train["gravity"])+gravity_dif, self.no_mf)
        self.qpa_mf = []
        self.pulse_mf = []
        self.resp_mf = []
        self.gravity_mf = []
    ...
    def generate_rules(self):
        ...
        for rule in rules[1:]:
            ...
            d=rule[3]
            e=rule[4]
            if [a,b,c,d] in alternative:
                continue
            elif [a,b,c,d] not in alternative:
