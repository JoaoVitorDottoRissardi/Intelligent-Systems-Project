...
class View:
    ...
    def __init__(self, model):
        self.model = model
        ##Define a posicao dos agentes
        self.posRobSearcher = None
        self.posRobHelper = None
    ...
    def draw(self):
        ...
        if self.posRobSearcher != None:
            self.board.listPlaces[self.posRobSearcher[0]][self.posRobSearcher[1]].setAgentSearcher(False)
            self.board.listPlaces[self.posRobSearcher[0]][self.posRobSearcher[1]].show()

        if self.posRobHelper != None:
            self.board.listPlaces[self.posRobHelper[0]][self.posRobHelper[1]].setAgentHelper(False)
            self.board.listPlaces[self.posRobHelper[0]][self.posRobHelper[1]].show()
        ## Desenha o robo na nova posicao, e mostra a mensagem do robo no lado
        self.board.listPlaces[self.model.agentSearcherPos[0]][self.model.agentSearcherPos[1]].setAgentSearcher(True)
        self.board.listPlaces[self.model.agentSearcherPos[0]][self.model.agentSearcherPos[1]].show()
        self.posRobSearcher = (self.model.agentSearcherPos[0], self.model.agentSearcherPos[1])

        self.board.listPlaces[self.model.agentHelperPos[0]][self.model.agentHelperPos[1]].setAgentHelper(True)
        self.board.listPlaces[self.model.agentHelperPos[0]][self.model.agentHelperPos[1]].show()
        self.posRobHelper = (self.model.agentHelperPos[0], self.model.agentHelperPos[1])
